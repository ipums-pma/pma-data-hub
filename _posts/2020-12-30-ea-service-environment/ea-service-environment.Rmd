...
hhf <- read_ipums_micro(
  ...
  select(PERSONID, EAID, YEAR, FPCURRUSE) %>% 
  mutate(
    across(everything(), ~lbl_na_if(
      .x,
      ~.lbl %in% c(
...
```{r, eval=F, include=F}
# sdp <- map(c("_inEA", "_outEA", "_all"), ~{
# 
#   name <- .x
# 
#   ea_filter <- function(data, name){
#     if(name == "_inEA"){filter(data, EAID == EASERVED)}
#     else if(name == "_outEA"){filter(data, EAID != EASERVED)}
#     else {filter(data)}
#   }
# 
#   sdp %>%
...
#       across(ends_with("OBS"), ~any(.x == 1), .names = "ANY_{.col}_INSTOCK"),
#       across(ends_with("OUT3MO"), ~any(.x == 1), .names = "ANY_{.col}"),
#       across(ends_with("OUTEV"), ~any(.x == 1), .names = "ANY_{.col}")
#     ) %>%
#     mutate(
#       NUM_METHODS_PROV = sum(c_across(ends_with("PROV")), na.rm = T),
#       NUM_METHODS_OUT = sum(c_across(ends_with("OBS_OUT")), na.rm = T),
...
#       NUM_METHODS_OUT3MO
#     ) %>% 
#     rename_with(~paste0(.x, name), -c(EASERVED, YEAR))
# })
# 
# sdp <- sdp %>%
#   reduce(full_join, by = c("EASERVED", "YEAR")) %>%
#   arrange(EASERVED, YEAR) %>%
#   mutate(across(everything(), ~replace_na(.x, 0)))
```

```{r, eval=F, include=F}
# sdp %>% summarise_all(sum) %>% str()

#We are currently losing any facility that was interviewed (CONSENTSQ == 1) and doesn't serve the ea that it's located in! This is a problem only once: EA 7554 contains only one facility (id 7582) and it does not serve the EA*
```
