...
To run the code in this post, you'll need the [tidyverse package](https://tidyverse.tidyverse.org/) from CRAN. See our post [Installing and Loading Useful R Packages](getPackages.html) for help.

```{r}
library(tidyverse)
```

To keep things simple, we'll focus on 2018 Nutrition data from Burkina Faso, which you can download at [pma.ipums.org](https://pma.ipums.org/pma-action/variables/group). See our post [Downloading an IPUMS Extract for R](getData.html) for help.
...
We could then take each child's mid-upper arm circumference (MUAC) [`KIDARMCIRCVAL`](https://pma.ipums.org/pma-action/variables/KIDARMCIRCVAL) as a simple proxy measure for growth and nutrition. Comparing children whose mothers received newborn feeding information to those whose mothers received none, no real difference seems to exist (at least not without additional control variables):

```{r, layout="l-body-outset"}
x_lab = "Child's mid-upper arm circumference (MUAC)"
legend_lab = str_wrap("Mother received newborn feeding information during antenatal care", 15)
